...
create database school_example ;
use school_example;
...
# Select distict examples from different years
select distinct * from SECTION where sYear > 2016;

select distinct * from SECTION where sYear >= 2016 and sYear < 2019;

# Unary Relational Operations: PROJECT
select distinct Section_id , Course_number from SECTION where sYear <> 2018;

select distinct Course_number from SECTION;

# Rename function using credit hours as example
select distinct Course_number as CNO , Course_name as CNOM from COURSE Where Credit_hours = 4;
...
# Except example
SELECT distinct Section_id FROM SECTION where Section_id not in (SELECT distinct Section_id FROM Grade_REPORT);

# Intersect
SELECT distinct Section_id FROM SECTION where Section_id in (SELECT distinct Section_id FROM Grade_REPORT);

# Union cross product
SELECT distinct STUDENT.sName , STUDENT.snumber , Grade_REPORT . Student_number FROM STUDENT CROSS JOIN Grade_REPORT;
...
ON S. Course_number = C. Course_number;
# Aggregate Functions and Grouping - count function
